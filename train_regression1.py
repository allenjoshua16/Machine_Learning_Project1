#!/usr/bin/env python3
"""
Train Regression (variant 1)
- Uses sepal_length and sepal_width to predict petal_length
- Implements 10% stratified test split on Iris species
- Trains Linear Regression with batch gradient descent + early stopping
- Saves fitted model, learning curve, predictions plot, and held-out test split
"""
...
from utils import mse, r2_score
...
def main() -> None:
    ...
    model.save("model_reg1.npz")
    print("Saved model to model_reg1.npz")
    ...
    plt.ylabel("MSE loss")
    plt.title("Learning Curve - Regression 1")
    plt.legend()
    plt.tight_layout()
    plt.savefig("regression1_learning_curve_mse.png", dpi=300)
    plt.close()
    print("Saved plot to regression1_learning_curve_mse.png")
    ...
    plt.title("Predicted vs. True - Regression 1")
    plt.tight_layout()
    plt.savefig("regression1_pred_vs_true.png", dpi=300)
    plt.close()
    print("Saved plot to regression1_pred_vs_true.png")

    np.savez(
        "regression1_test_split.npz",
        X_test=X_test,
        y_test=y_test,
        features=np.array(features, dtype=object),
        target=np.array(target, dtype=object),
    )
    print("Saved held-out test split to regression1_test_split.npz")
