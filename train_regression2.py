#!/usr/bin/env python3
"""
Train Regression (variant 2)
- Uses petal_length to predict sepal_length
- Applies 10% stratified test split on Iris species
- Trains Linear Regression with batch gradient descent + early stopping
- Persists model, diagnostics, and held-out test split
"""
...
def main() -> None:
    ...
    model.save("model_reg2.npz")
    print("Saved model to model_reg2.npz")
    ...
    plt.ylabel("MSE loss")
    plt.title("Learning Curve - Regression 2")
    plt.legend()
    plt.tight_layout()
    plt.savefig("regression2_learning_curve_mse.png", dpi=300)
    plt.close()
    print("Saved plot to regression2_learning_curve_mse.png")
    ...
    plt.title("Predicted vs. True - Regression 2")
    plt.tight_layout()
    plt.savefig("regression2_pred_vs_true.png", dpi=300)
    plt.close()
    print("Saved plot to regression2_pred_vs_true.png")

    np.savez(
        "regression2_test_split.npz",
        X_test=X_test,
        y_test=y_test,
        features=np.array(features, dtype=object),
        target=np.array(target, dtype=object),
    )
    print("Saved held-out test split to regression2_test_split.npz")
