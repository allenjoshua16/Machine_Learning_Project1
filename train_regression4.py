#!/usr/bin/env python3
"""
Train Regression (variant 4)
- Uses petal_length and sepal_width to predict petal_width
...
def main() -> None:
    ...
    model.save("model_reg4.npz")
    print("Saved model to model_reg4.npz")
    ...
    plt.ylabel("MSE loss")
    plt.title("Learning Curve - Regression 4")
    plt.legend()
    plt.tight_layout()
    plt.savefig("regression4_learning_curve_mse.png", dpi=300)
    plt.close()
    print("Saved plot to regression4_learning_curve_mse.png")
    ...
    plt.xlabel("True petal_width")
    plt.ylabel("Predicted petal_width")
    plt.title("Predicted vs. True - Regression 4")
    plt.tight_layout()
    plt.savefig("regression4_pred_vs_true.png", dpi=300)
    plt.close()
    print("Saved plot to regression4_pred_vs_true.png")

    np.savez(
        "regression4_test_split.npz",
        X_test=X_test,
        y_test=y_test,
        features=np.array(features, dtype=object),
        target=np.array(target, dtype=object),
    )
    print("Saved held-out test split to regression4_test_split.npz")
